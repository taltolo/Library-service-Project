...
public class InventoryController {
						...
						alert.showAndWait().ifPresent(type -> {
							if (type == okButton) {
								...
								if(removebook.getCurrentlyonloan()>0) 
									Main.client.HomePageController.displayAlert(AlertType.INFORMATION, "Remove a Book", "Remove a Book", "There still have a "+removebook.getCurrentlyonloan()+ " books in loan.\nPlease return them before removing.");

								else
									try {
										//Main.client.librarianController.RemoveBook(removebook);
										Main.librariancontroller.RemoveBook(removebook);
									} catch (IOException e) {
	...
	void SearchHandler(ActionEvent event) {
		...
		//Msg msg=new Msg("SELECT * FROM books WHERE idbook = "+idsearch +";", Command.searchBookFromInventory);
	...
	public void initialize() {
		...
		Main.client.inventoryController=this;
